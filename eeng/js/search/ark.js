...
var contentListId = "ark";						// 자동완성 Content List <ul> 의 id을 설정한다
...
var target = "mobile";								// ARK 웹서버 설정파일의 목록에 있는 추천어 서비스 대상을 지정한다.
var charset = "utf-8";							// 인코딩 설정 (인코딩이 utf-8이 아닐 경우 8859_1 로 설정해야함)
var datatype = "json";							// 반환받을 Data의 타입을 설정. XML 과 JSON이 가능 (xml | json)
var arkPath = "./ark";						// 자동완성 경로
//var transURL = arkPath + "/ark_trans.jsp";		// trans 페이지의 URL을 설정한다.
var transURL = "/apps/search/ark.do";		// trans 페이지의 URL을 설정한다.
var tempQuery = "";
...
//				hideArk();
	...
	jQuery(queryId).keyup(function(event) {
		if(event.keyCode == 8 && jQuery(this).val() == "") {
			if($("#chkmykeyword").val() != 'nomykeyword'){
				jQuery(".txtMyKeyword").show();
		 	}
			jQuery(".txtRecommend").hide();
			
			/*
			var msg = "<li>현재 검색어 &nbsp;<strong>자동 추천 기능</strong>을 사용하고 있습니다.</li>";
			msg += "<li>검색어 입력시 자동으로 관련어를 추천합니다.</li>";
...
//			hideArk();
	...
	jQuery(document).click(function(event) {
		...
		/*
		else if (!jQuery(event.target).is(".btn_txt_delete")) {
			hideArk();
		}
		*/
	});
...
//					}
					...
					jQuery.each(result.items, function(num,item){
						...
						str += "<li>";
						str += 		"<a href=\"javascript:doKeyword('" + item.keyword + "');\">" + "&nbsp;&nbsp;" + replaceAll(item.hkeyword,'12px', '17px') +"</a>";;
						//if(specialKeyYn(item.keyword) == 'Y') {
							//var spUrl = getSpecialValue(item.keyword);
							//if(spUrl != '') {
								//str += "<a href=\"\" class=\"btn_sch_go\" onclick=\"goSpecial('" +item.linkurl+ "');\">";
							//}
						//}
						str += "</li>";
					});
			...
			str += "<p class=\"txt_close\"><a href=\"javascript:autoCompleteClose()\">닫기</a></p>";

			if ((totalFwCount + totalRwCount) == 0) {
				jQuery("#" + contentListId).html("<ul><li>해당 단어로 시작하는 검색어가 없습니다.</li></ul><p class=\"txt_close\"><a href=\"javascript:autoCompleteClose()\">닫기</a></p>");
			} else {
...
//	}
 	jQuery(".txtMyKeyword").hide();
 	jQuery(".txtRecommend").show();
